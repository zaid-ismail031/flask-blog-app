...
@login_required
def myblog():
    error = "These are your blogs!"
    bloginfo = db.execute("SELECT post, title, date, time FROM posts WHERE user_id = :id;", id=session["user_id"])
    if not bloginfo:
        error = "You have no blogs!"
        return render_template("myblog.html", error = error, length = 0)
    length = []
    ...
    return render_template("myblog.html", bloginfo = bloginfo, length = length, error = error)
...
@login_required
def recent():
    info = db.execute("SELECT username, title, post, date, time FROM users JOIN posts on posts.user_id = users.id ORDER BY date DESC, time DESC;")
    length = []
...
def search():
    if request.method == "POST":
        ...
        searchdb = db.execute("SELECT username, bio FROM users JOIN profile on profile.user_id = users.id WHERE username = :username;", username = search)
        if not searchdb:
            error = "User does not exist or user does not have a bio."
            return render_template("search.html", error = error)
        username = searchdb[0]["username"]
        bio = searchdb[0]["bio"]
        return render_template("searched.html", bio=bio, username=username)
    else:
        error = "Search for a user..."
        return render_template("search.html", error = error)